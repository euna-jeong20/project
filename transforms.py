...
class ResizeImage(nn.Module):
    ...
    def forward(
        ...
        image = F.resize(image, [self.new_height, self.new_width], interpolation=self.interpolation)

        if target is not None:
            target["boxes"][:, 0::2] *= self.new_width / orig_width
            target["boxes"][:, 1::2] *= self.new_height / orig_height
            if "masks" in target:
                target["masks"] = F.resize(
                    target["masks"], [self.new_height, self.new_width], interpolation=InterpolationMode.NEAREST
                )
